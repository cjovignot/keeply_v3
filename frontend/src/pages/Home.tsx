      ...
      <div className="flex flex-col items-center px-6 pt-8 text-white">
        ...
        <section className="w-full max-w-4xl mt-6 grid md:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <ul className="text-gray-300 mb-6 space-y-1 text-sm">
            ...
            >
              <Link to="/login">
                <motion.button
                  whileTap={{ scale: 0.95 }}
                  className="flex items-center gap-2 px-6 py-3 text-black font-semibold bg-yellow-400 rounded-full hover:bg-yellow-500"
                >
                  Commencer maintenant <ArrowRight size={18} />
                </motion.button>
              </Link>
              <p className="text-xs text-gray-400 mt-2">
                Gratuit · Sans App Store · Fonctionne hors ligne
              </p>
            </motion.div>
          </div>
        </section>
        ...
        <section className="w-full max-w-4xl mt-12 mb-14 grid md:grid-cols-3 gap-8">
          <Feature
            ...
            text="Visualisez immédiatement ce que vous avez et où cela se trouve."
            image={storageBoxes}
          />
          <Feature
            title="Vous savez ce que vous possédez"
            text="Objets, quantités, photos : plus de doublons inutiles."
            image={mysteryBox}
          />
          <Feature
            title="Vous retrouvez sans ouvrir"
            text="Un scan de QR code suffit pour connaître le contenu d’une boîte."
            image={qrCodePhone}
          />
        </section>
        ...
        <section className="w-full max-w-4xl mt-20 mb-20">
          ...
          <div className="grid md:grid-cols-3 gap-10">
            {["Photographiez", "Étiquetez", "Scannez et retrouvez"].map(
              (title, i) => (
                <motion.div
                  key={i}
                  initial={{ opacity: 0, y: 60 }}
...
function Feature({ title, text, image }) {
  return (
    <div className="p-6 bg-gray-900 border border-gray-800 rounded-2xl shadow-lg">
      ...
      <img src={image} alt="Illustration" className="w-full mt-6 rounded-md" />
    </div>
  );
}
